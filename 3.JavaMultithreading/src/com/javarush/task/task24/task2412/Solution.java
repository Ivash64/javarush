    ...
    public static void sort(List<Stock> list) {
        Collections.sort(list, new Comparator<Stock>() {
            public int compare(Stock stock1, Stock stock2) {
                ...
                int comp;
                if ((comp = name1.compareTo(name2)) != 0)
                    return comp;
                Integer date1 = ((Date)stock1.get("date")).getDate();
                Integer date2 = ((Date)stock2.get("date")).getDate();
                if ((comp = date1.compareTo(date2)) != 0)
                    return -1*comp;
                Double value1 = getMoney(stock1);
                Double value2 = getMoney(stock2);
                if ((comp = value1.compareTo(value2)) != 0)
                    return -1*comp;
                return 0;
            }

            public double getMoney(Stock stock) {
                if (stock.containsKey("open") && stock.containsKey("last")) {
                    return  (double) stock.get("last") - (double) stock.get("open");
                }
                else if (stock.containsKey("change")){
